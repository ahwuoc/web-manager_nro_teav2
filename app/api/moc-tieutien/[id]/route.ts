...
export async function PUT(
    ...
    try {
        ...
        const updated = await prisma.moc_tieutien.update({
        ...
        console.error('Error updating moc_tieutien:', error);
...
export async function DELETE(
    ...
    try {
        const { id } = await params;
        await prisma.moc_tieutien.delete({ where: { id: parseInt(id) } });
        return NextResponse.json({ success: true });
    } catch (error) {
        console.error('Error deleting moc_tieutien:', error);
